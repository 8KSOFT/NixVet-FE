// forked from design-sync lib/bundle.mjs — NixVet-FE is a Next.js app, not a
// standalone design-system package. The synthesized entry transitively pulls
// in Next.js client internals (next/link, next/image, next/navigation, and
// app modules like src/lib/axios.ts reading process.env.NEXT_PUBLIC_*) that
// reference a bare `process` global (process.env.__NEXT_*, process.platform,
// process.nextTick, …) — Next's webpack provides that at build time, but
// esbuild's browser-platform bundle leaves it unresolved, crashing EVERY
// preview at bundle-init with "ReferenceError: process is not defined". The
// only change from upstream is adding `inject: [PROCESS_SHIM]` to
// sharedBuildOptions (see process-shim.mjs in this same directory) — output
// IIFE shape, header stamp, and footer are untouched, so the app's
// self-check contract is unaffected.
...
const PROCESS_SHIM = fileURLToPath(new URL('./process-shim.mjs', import.meta.url));
