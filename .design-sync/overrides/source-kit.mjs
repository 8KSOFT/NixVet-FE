// forked from design-sync lib/source-kit.mjs — the synth-entry fallback (no
// dist/) wrote `export * from <file>;` for every source file. In ES modules
// `export *` does NOT re-export a file's DEFAULT export — only named exports.
// NixVet-FE's shared/app-level components (AppProviders, Logo,
// LanguageSwitcher, LegalDocumentShell) are `export default function X()`,
// so they were silently missing from window.NixVetUI even though the
// component LIST correctly discovered them via deriveComponentsFromSrc
// (which recovers the declared name from the default-export declaration
// node). Concretely this broke cfg.provider (AppProviders resolved to
// `undefined` on the bundle, so every preview's provider wrapper failed) —
// masked in aggregate by the floor-card's empty-root fallback, but visible
// as [BUNDLE_EXPORT] not-a-component and one outright [RENDER_ERRORS]
// ("Element type is invalid ... got: undefined"). The only change from
// upstream: the synth-entry writer also emits `export { default as Name }
// from <file>;` for files with a recoverable default-exported PascalCase
// name. Everything else (dist-entry resolution, src/ enrichment, grouping)
// is untouched.
...
// NixVet-FE addition: for each candidate file, recover its default export's
// declared name (if any) so the synth entry can re-export it by name — `export *`
// silently drops default exports.
function defaultExportNameOf(project, p) {
  const sf = project.addSourceFileAtPathIfExists(p);
  if (!sf) return null;
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const lines = comps.flatMap((p) => {
      const out = [`export * from ${JSON.stringify(p)};`];
      const defName = defaultExportNameOf(defaultProject, p);
      if (defName) {
        out.push(`export { default as ${defName} } from ${JSON.stringify(p)};`);
        defaultCount++;
      }
      return out;
    });
    ...
    synthEntry = true;
    console.error(
      `[NO_DIST] no built entry — synthesizing from ${comps.length} src files` +
        (defaultCount ? ` (${defaultCount} default export(s) re-exported by name)` : '') +
        ` (run the package's build for best results)`,
    );
  }
