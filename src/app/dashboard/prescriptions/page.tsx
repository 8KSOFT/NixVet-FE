...
const COMMON_VACCINES = [
  { group: 'Caninos', items: ['V8', 'V10', 'Antirrábica', 'Gripe Canina (Bordetella)', 'Giardíase', 'Leishmaniose (Leishmania)'] },
  { group: 'Felinos', items: ['Tríplice Felina (HVC, HCC, Panleucopenia)', 'Quádrupla Felina', 'Antirrábica (felino)', 'FIV/FeLV'] },
  { group: 'Equinos', items: ['Tétano', 'Influenza Equina', 'Herpesvírus Equino'] },
...
export default function PrescriptionsPage() {
  ...
  const [prescriptionType, setPrescriptionType] = useState<'receita' | 'solicitacao_cirurgia' | 'vacinas'>(
    'receita',
  );
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (prescriptionType === 'receita') {
        ...
        payload.medications = selectedVaccines.map((name) => ({ name, via: 'Subcutânea' }));
      } else {
        payload.medications = [];
  ...
  return (
    <div>
      ...
      <Dialog open={modalVisible} onOpenChange={setModalVisible}>
        <DialogContent
          className="flex flex-col w-full max-w-2xl h-[96dvh] max-h-[96dvh] p-0 gap-0 sm:h-[92dvh]"
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    value={field.value}
                    onValueChange={(v) => { field.onChange(v); handlePatientChange(v); }}
                  >
              ...
              <div className="space-y-3">
                  ...
                  <div className="border rounded-lg p-3 flex flex-wrap gap-2 bg-muted/30">
                    {selectedVaccines.map((v) => (
                      <Badge key={v} variant="secondary" className="flex items-center gap-1.5 py-1 px-2 text-sm">
                        {v}
                        <button type="button" onClick={() => setSelectedVaccines((prev) => prev.filter((x) => x !== v))} className="hover:text-destructive">
                          <X className="w-3 h-3" />
                        </button>
                      </Badge>
                    ))}
                ...
                <div className="border rounded-lg divide-y max-h-64 overflow-y-auto bg-background">
                  {COMMON_VACCINES.map((group) => (
                    <div key={group.group}>
                      <div className="px-4 py-2 bg-muted/50 text-xs font-semibold text-muted-foreground uppercase tracking-wide">{group.group}</div>
                      {group.items.map((vac) => (
                        <label key={vac} className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer hover:bg-muted/50 text-sm transition-colors ${selectedVaccines.includes(vac) ? 'bg-primary/5' : ''}`}>
                          <input
                            type="checkbox"
                            checked={selectedVaccines.includes(vac)}
                            onChange={() => setSelectedVaccines((prev) => prev.includes(vac) ? prev.filter((x) => x !== vac) : [...prev, vac])}
                ...
                <p className="text-xs text-muted-foreground">Adicione o número do lote, fabricante ou nome comercial no campo acima se necessário.</p>
              </div>
            )}
              ...
              <div className="space-y-2">
                ...
                <div className="border rounded-lg divide-y max-h-48 overflow-y-auto bg-background">
                  {surgicalProcedures
                    ...
                    .map((s) => (
                      <label key={s.id} className="flex items-center gap-3 px-4 py-2.5 cursor-pointer hover:bg-muted/50 text-sm">
                        <input
                          type="checkbox"
            ...
            {prescriptionType === 'receita' && (
              <div className="space-y-3">
                ...
                {fields.map((field, index) => {
                  ...
                  return (
                    <div key={field.id} className="border rounded-xl p-4 bg-muted/30 space-y-3">
                      ...
                      <div className="flex gap-2 items-start">
                                ...
                                bularioOptions.map((item) => (
                                  <button
                                    ...
                                    onClick={() => {
                                      ...
                                      setMedInputValues((prev) => ({ ...prev, [index]: item.title }));
                                      setActiveMedIndex(null);
                                      setBularioOptions([]);
                                    }}
                                  >
                                    <span className="font-medium">{item.title}</span>
                                    {item.subtitle && <span className="text-muted-foreground ml-2 text-xs">{item.subtitle}</span>}
                                  </button>
                                ))
                              )}
                            </div>
                        ...
                        {bularioId && (
                          <Button type="button" variant="outline" size="sm" className="h-10 shrink-0" onClick={() => openBularioDetail(bularioId)}>
                            <Info className="w-4 h-4 mr-1" /> Bula
                          </Button>
                        )}
                        <Button type="button" variant="ghost" size="icon" className="h-10 w-10 text-destructive shrink-0" onClick={() => remove(index)}>
                          <X className="w-4 h-4" />
                        </Button>
                      </div>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        ...
                        <div className="space-y-1">
                          ...
                          <Controller control={control} name={`medications.${index}.use_type`}
                            render={({ field }) => (
                              <Select value={field.value ?? ''} onValueChange={field.onChange}>
                                <SelectTrigger className="h-9"><SelectValue placeholder="Uso" /></SelectTrigger>
                                <SelectContent>
                        ...
                        <div className="space-y-1">
                          ...
                          <Controller control={control} name={`medications.${index}.form_of_administration`}
                            render={({ field }) => (
                              <Select value={field.value ?? ''} onValueChange={field.onChange}>
                                <SelectTrigger className="h-9"><SelectValue placeholder="Forma" /></SelectTrigger>
                                <SelectContent>
                                  {FORM_ADMIN_OPTIONS.map((o) => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
                                </SelectContent>
                              </Select>
                            )} />
                        </div>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <div className="space-y-1">
                          ...
                          <Controller control={control} name={`medications.${index}.dosage`}
                            render={({ field }) => <Input className="h-9" placeholder="Ex: 1 comp." {...field} />} />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs text-muted-foreground">Frequência</Label>
                          <div className="flex gap-1">
                            <Controller control={control} name={`medications.${index}.frequency_value`}
                              render={({ field }) => <Input type="number" min="1" className="h-9 w-16 shrink-0" placeholder="12" {...field} />} />
                            <Controller control={control} name={`medications.${index}.frequency_unit`}
                              render={({ field }) => (
                                <Select value={field.value ?? ''} onValueChange={field.onChange}>
                                  <SelectTrigger className="h-9 flex-1"><SelectValue placeholder="Un." /></SelectTrigger>
                                  <SelectContent>
                                    {FREQUENCY_UNIT_OPTIONS.map((o) => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
                                  </SelectContent>
                                </Select>
                              )} />
                          </div>
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex gap-1">
                            <Controller control={control} name={`medications.${index}.duration_value`}
                              render={({ field }) => <Input type="number" min="1" className="h-9 w-16 shrink-0" placeholder="7" {...field} />} />
                            <Controller control={control} name={`medications.${index}.duration_unit`}
                              render={({ field }) => (
                                <Select value={field.value ?? ''} onValueChange={field.onChange}>
                                  <SelectTrigger className="h-9 flex-1"><SelectValue placeholder="Un." /></SelectTrigger>
                                  <SelectContent>
                                    {DURATION_UNIT_OPTIONS.map((o) => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
                                  </SelectContent>
                                </Select>
                              )} />
                          </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="space-y-1">
                          ...
                          <Controller control={control} name={`medications.${index}.usage_description`}
                            render={({ field }) => <Textarea rows={2} placeholder="Ex: Administrar com alimento." className="resize-none" {...field} />} />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs text-muted-foreground">Observações</Label>
                          <Controller control={control} name={`medications.${index}.observations`}
                            render={({ field }) => <Textarea rows={2} placeholder="Obs. adicionais" className="resize-none" {...field} />} />
                        </div>
                      </div>
                    </div>
                  );
                ...
                <Button
                  ...
                  onClick={() => append({ name: '', via: '', concentration: '', use_type: '', form_of_administration: '', dosage: '', frequency_value: '', frequency_unit: 'horas', duration_value: '', duration_unit: 'dias', usage_description: '', observations: '' })}
                >
            ...
            <div className="space-y-1.5">
              ...
              <Controller control={control} name="observations"
                render={({ field }) => <Textarea rows={3} placeholder="Orientações adicionais ao tutor..." className="resize-none" {...field} />} />
            </div>
          </div>
          ...
          <DialogFooter className="px-6 py-4 border-t shrink-0 flex flex-row gap-3 justify-end">
            ...
            <Button type="submit" form="prescription-form" className="h-10 bg-primary hover:bg-blue-700 text-white" onClick={handleSubmit(onSubmit)}>
              Gerar prescrição
            </Button>
          </DialogFooter>
        </DialogContent>
