...
export default function TeamPage() {
  ...
  const onSubmit = async (values: UserFormValues) => {
    try {
      const payload: Record<string, string> = { ...values } as Record<
        string,
        string
      >;
      if (editingId) {
  ...
  if (forbidden) {
    return (
      <div>
        <h1 className="text-2xl font-heading font-bold mb-4 flex items-center gap-2">
          {t("team.title")}
        </h1>
    ...
    <div>
      ...
      <Dialog open={modalVisible} onOpenChange={setModalVisible}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingId ? t("team.editTitle") : t("team.newTitle")}
            </DialogTitle>
          </DialogHeader>
