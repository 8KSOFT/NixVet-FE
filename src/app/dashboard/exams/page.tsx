...
function ExamRequestsContent() {
      ...
      for (const name of selectedExams) {
        ...
        if (trimmed && !catalogNames.has(trimmed) && defaultAreaId) {
          try {
            ...
            console.warn(
              "Não foi possível adicionar exame ao catálogo:",
              trimmed,
            );
          }
        }
  ...
  const getPatient = (record: ExamRequest) =>
    record.consultation?.patient ?? record.patient;

  const [emailModalVisible, setEmailModalVisible] = useState(false);
  const [selectedExamRequest, setSelectedExamRequest] =
    useState<ExamRequest | null>(null);
            ...
            <Table className="min-w-full border-collapse bg-white text-sm">
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="border-l border-slate-200 px-3 py-2 text-left text-[11px] uppercase tracking-[0.12em] text-slate-600">Exames</TableHead>
                    ...
                    <TableCell className="border border-slate-200 px-3 py-3 text-slate-600">
                      {record.requested_exams?.length
                        ? record.requested_exams
                            .map((name) => {
                              const catalog = examsFromCatalog.find(
                                (e) => e.name === name,
                              );
                              return catalog?.area?.name
                                ? `${catalog.area.name} - ${name}`
                                : name;
                            })
                            .join(", ")
                        : "—"}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">
                Selecione da lista, digite para filtrar ou adicione texto livre
                (Enter)
              </span>
            </div>
              ...
              {showExamDropdown &&
                (filteredExamOptions.length > 0 || examInput.trim()) && (
                  <div className="absolute z-20 top-full left-0 right-0 bg-white border rounded-lg shadow-lg max-h-52 overflow-y-auto mt-1">
                    ...
                    {examInput.trim() &&
                      !examsFromCatalog.find(
                        (e) => e.name === examInput.trim(),
                      ) && (
                        <button
                          type="button"
            ...
            <p className="text-xs text-muted-foreground">
              Você pode adicionar exames não cadastrados digitando o nome e
              pressionando Enter.
            </p>
          </div>
      ...
      <DashboardCreateFormDialog
        ...
        footer={
          <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <Button
              variant="outline"
              onClick={() => setAddExamModalVisible(false)}
            >
              Cancelar
            </Button>
            <Button
